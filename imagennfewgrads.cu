//can add in the relu and backwards gpu versions to  make this run faster to test against the regular imagenn.
//make those changes in subsetlayer in nnfromscratch.cu file
#include <fstream>
#include <time.h>
#include <random>
#include <chrono>
#include "nnfromscratch.cu"
...
int main()
{
    ...
    Subsetlayer ld1(n_inputs, n_neurons, false, num_samples, randweilabels[0], true);
    Relu_Activation relu(num_samples, n_neurons);
    Subsetlayer ld2(n_neurons, n_neurons, false, num_samples, randweilabels[1], true);
    Relu_Activation rel2(num_samples, n_neurons);
    Subsetlayer ld3(n_neurons, num_classes, false, num_samples, randweilabels[2]);
    Activation_softmaxgpu soft(num_samples, num_classes);
    ...
    for (size_t i = 0; i < num_iters; i++)
    {
        ...
        ld1.gpu_forward_relu(trainimages);
        // relu.forward(ld1.output);
        ld2.gpu_forward_relu(ld1.output);
        // rel2.forward(ld2.output);
        ld3.gpu_forward(ld2.output);

        soft.forwardsemiopt(ld3.output);
        ...
        if (i % check_iter == 0 )
        {
            ...
            //  mat_print(ld3.output);
            for (size_t l = 0; l < loss.negative_log_likelihood.cols; l++)
            {
                total_loss += MAT_AT(loss.negative_log_likelihood, 0, l);
            }
            float avg_loss = total_loss/num_samples;
            printf("average losss: %f\n", avg_loss);
        }
        soft.GPU_backward(loss.dinputs);
        ld3.gpu_backward(soft.dinputs);
        ld2.backward_relu(ld3.dinputs);
        ld1.backward_relu(ld2.dinputs);
    ...
    mat_free(trainimages);
    mat_free(trainlabels);
